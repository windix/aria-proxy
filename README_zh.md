...
- **自动化重命名规则**: 根据可选配置 `data/rename-rules.yaml`，以无感方式自动替换并重命名要保存的文件。
...
您可以通过在 `data/` 目录下创建一个 `rename-rules.yaml` 文件来对传入的输出文件名进行自动化文本替换。

该文件应该是一个包含字符串对 `["TARGET", "REPLACEMENT"]`（目标字符串和替换字符串）的 YAML 数组：
