...
describe('Dashboard Database API', () => {
  ...
  it('fetches all requests', async () => {
    db.prepare('INSERT INTO requests (url, status) VALUES (?, ?)').run('http://foo.com/bar', 'pending');
  ...
  it('exports pending requests exclusively and updates string layout natively', async () => {
    db.prepare('INSERT INTO requests (url, out_filename, options_json) VALUES (?, ?, ?)')
      .run('http://example.com/test.bin', 'test.bin', JSON.stringify({ out: 'test.bin', header: 'Custom: test' }));
    ...
    expect(records[0].status).toBe('exported'); // Validated that marking occurred
  });

  it('clears all requests from the db entirely', async () => {
    db.prepare('INSERT INTO requests (url) VALUES (?)').run('http://delete.me');
    
    let current = db.prepare('SELECT count(*) as count FROM requests').get();
    expect(current.count).toBe(1);

    const res = await request(app).post('/api/requests/clear-all');
    expect(res.status).toBe(200);

    current = db.prepare('SELECT count(*) as count FROM requests').get();
